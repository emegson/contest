...
int bt_split_child(bt_node *parent, int index, bt_node *child) {
    ...
    for(j = parent->num_keys; j >= index + 1; j--){
    ...
    for(j = parent->num_keys-1; j >= index + 1; j--){
...
bt_node* bt_split(bt_node *x) {
    ...
    */
    int contador;
    int mediana = x->keys[x->t];
    bt_node* nodeY;
    nodeY = bt_new_node(x->t, x->is_leaf);
    if(x->is_leaf){
        for(contador = (x->t)+1; contador < (2*(x->t)- 1); contador++){
            bt_insert_non_full(nodeY,x->keys[contador]);
        }
    }
    printf("nodeY -> num_keys = %d\n", nodeY->num_keys);
    return nodeY;
}
